...
export const courseContent = [
  ...
  },
  {
    ...
    quiz: [
        ...
        "What is the command to switch a git remote from HTTPS to SSH?"
    ],
    pages: [
        {
        ...
        {
            title: "Lesson 1.6: Setting Up SSH for GitHub",
            content: [
                "While you can use HTTPS to push code to GitHub, it requires a Personal Access Token instead of a password, which can be cumbersome. Using SSH is a more secure and convenient method that solves `Authentication failed` errors.",
                "SSH (Secure Shell) uses a key pair: a private key that stays on your computer and a public key that you upload to GitHub. This allows GitHub to verify your identity without needing a password.",
                "First, generate a new SSH key by opening your terminal and running this command, replacing the email with your own:",
                { type: "code", language: "bash", code: 'ssh-keygen -t ed25519 -C "your_email@example.com"' },
                "Next, copy your public key to your clipboard. You can display it by running: `cat ~/.ssh/id_ed25519.pub`",
                "Finally, go to your GitHub account settings, navigate to 'SSH and GPG keys', and add your new public key.",
                "If your project is already using an HTTPS remote URL, you must switch it to SSH to fix authentication errors. First, find your SSH URL on your GitHub repository page (it will look like `git@github.com:USERNAME/REPO.git`). Then, in your terminal, run:",
                { type: 'code', language: 'bash', code: 'git remote set-url origin git@github.com:USERNAME/REPO.git' },
                "Now, when you run `git push`, it will use your SSH key for authentication, and you won't see the password errors anymore!"
            ],
            researchPrompt: "What is the `ssh-agent` and how can it be used to manage your SSH keys without having to enter your passphrase every time?"
        }
    ]
  },
  {
